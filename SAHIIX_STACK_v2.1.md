...
┌─────────────────────────────────────────────────────────────────┐
│  L7  EXPERIENCE                                                  │
│      sahiixx-os (React/tRPC) · portfolio · Jarvis voice · CLI    │
├─────────────────────────────────────────────────────────────────┤
...
│      QualificationAgent · Discovery · RealEstate adapters        │
├─────────────────────────────────────────────────────────────────┤
│  L4  AGENTIC HARNESS                                             │
...
├─────────────────────────────────────────────────────────────────┤
│  L0  INFRA                                                       │
│      Cloudflare · Azure · Termux edge · Docker / GHCR            │
└─────────────────────────────────────────────────────────────────┘
```
...
| LeadCapture / GeoMatch / Scheduling / Reporting | Spec + E2E contracts; code TBD |
...
2. **Register** QualificationAgent in `_SPECIALIZED_ADAPTERS` + routing rule (if not already)
3. **Wire** NEXUS WhatsApp → `lead.created` event on MessageBus
4. **Ship** GeoMatchAgent (consume `lead.qualified`, use `adapters/realestate/`)
5. **Expose** `/api/opa/lead/qualify` (or equivalent) so E2E live test un-skips
6. **Harden** proxy → OPA path (proxy as front door for agent calls)
7. **Publish** this doc as the single source of truth; link from profile README
...
**SAHIIX Stack v2.1 — Autonomous AIOS for revenue + agents.**
